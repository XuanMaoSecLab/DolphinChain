...
BUILD_FLAGS = -ldflags "-X github.com/XuanMaoSecLab/DolphinChain/version.GitCommit=`git rev-parse --short=8 HEAD`"
...
draw_deps:
	...
	@goviz -i github.com/XuanMaoSecLab/DolphinChain/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
...
rpc-docs:
	...
	godoc2md -template rpc/core/doc_template.txt github.com/XuanMaoSecLab/DolphinChain/rpc/core | grep -v -e "pipe.go" -e "routes.go" -e "dev.go" | sed 's,/src/target,https://github.com/XuanMaoSecLab/DolphinChain/tree/master/rpc/core,' >> $(DESTINATION)
...
build-docker:
	...
	docker build --label=tendermint --tag="XuanMaoSecLab/DolphinChain" DOCKER
